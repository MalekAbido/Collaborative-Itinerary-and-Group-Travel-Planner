...
<head>
    ...
    <title>Manage Members - VoyageSync</title>
...
<body class="bg-surface overflow-y-auto">
    ...
    <main class="max-w-[900px] mx-auto mt-[100px] px-6 pb-12">
        ...
        <div class="bg-surface-container-lowest border border-outline-variant rounded-xl shadow-sm p-6 mb-8">
            ...
            <form action="/itinerary/members/invite/<?= htmlspecialchars($trip['itineraryId'] ?? '') ?>" method="POST" class="flex flex-col sm:flex-row items-end gap-4">
                <div class="flex-1 w-full">
                    <label class="block text-label-caps uppercase text-on-surface-variant mb-2" for="email">User Email Address</label>
                    <div class="relative">
                ...
                <div class="w-full sm:w-auto">
                    <label class="block text-label-caps uppercase text-on-surface-variant mb-2" for="role">Assign Role</label>
                    <select id="role" name="role" class="w-full sm:w-48 rounded-lg border border-outline-variant bg-surface-container-lowest px-4 py-2.5 text-body-sm text-on-surface focus:border-primary focus:ring-2 focus:ring-primary/30 focus:outline-none transition">
                        <option value="Member">👤 Member</option>
                        <option value="Editor">✏️ Editor</option>
                    </select>
                </div>
                <button type="submit" class="w-full sm:w-auto inline-flex items-center justify-center gap-2 rounded-lg bg-primary text-on-primary font-semibold text-body-sm px-6 py-2.5 shadow-sm hover:bg-on-primary-fixed-variant transition h-[42px]">
        ...
        <div class="bg-surface-container-lowest border border-outline-variant rounded-xl shadow-sm p-6">
            ...
            <div class="divide-y divide-outline-variant">
                ...
                <div class="hidden sm:grid grid-cols-[1fr_120px_120px_100px] gap-4 py-3 text-label-caps uppercase text-outline">
                    <span>Member</span><span class="text-center">Role</span><span>Joined</span><span class="text-right">Actions</span>
                </div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-[1fr_120px_120px_100px] gap-4 py-4 sm:py-3 items-center">
                            ...
                            <div class="text-left sm:text-center">
                                <?php if ($member['role'] === 'Leader'): ?>
                                    <span class="inline-flex items-center rounded-full bg-primary-fixed px-3 py-1 text-label-xs font-bold uppercase text-primary">👑 Leader</span>
                                ...
                                <?php else: ?>
                                    <span class="inline-flex items-center rounded-full bg-surface-container-highest px-3 py-1 text-label-xs font-bold uppercase text-outline">👤 Member</span>
                                <?php endif; ?>
                            </div>
                            ...
                            <div class="flex justify-start sm:justify-end gap-2">
                                <?php if ($member['role'] !== 'Leader'): ?>
                                    <form action="/itinerary/members/updateRole/<?= htmlspecialchars($trip['itineraryId']) ?>" method="POST" class="inline">
                                    ...
                                    <form action="/itinerary/members/remove/<?= htmlspecialchars($trip['itineraryId']) ?>" method="POST" class="inline" onsubmit="return confirm('Remove this user from the trip?');">
                                        <input type="hidden" name="memberId" value="<?= $member['id'] ?>">
