    ...
    <main class="max-w-[900px] mx-auto mt-[100px] px-6 pb-12">
        ...
        <div class="bg-surface-container-lowest border border-outline-variant rounded-xl shadow-sm p-6 mb-8">
            ...
            <div class="flex items-center justify-between mb-4">
                ...
                <?php if (isset($data['currentUserRole']) && ($data['currentUserRole'] === 'Leader' || $data['currentUserRole'] === 'Organizer')): ?>
                <button onclick="copyInviteLink('<?= htmlspecialchars($data['generalLink']) ?>')"
            ...
            <form action="/itinerary/members/invite/<?= htmlspecialchars($data['trip']['itineraryId'] ?? '') ?>" method="POST"
        ...
        <?php if (!empty($data['pendingInvites'])): ?>
        <div class="bg-surface-container-lowest border border-outline-variant rounded-xl shadow-sm p-6 mt-8">
            ...
            <div class="divide-y divide-outline-variant">
                <div class="hidden sm:grid grid-cols-[1fr_120px_140px] gap-4 py-3 text-label-caps uppercase text-outline">
                    <span>Invited Email</span><span class="text-center">Role</span><span class="text-right">Share Link</span>
                </div>

                <?php foreach ($data['pendingInvites'] as $invite): ?>
                
                <?php $joinLink = $data['appUrl'] . "/join/" . $invite['token']; ?>
                
                <div class="grid grid-cols-1 sm:grid-cols-[1fr_120px_140px] gap-4 py-4 sm:py-3 items-center">

                    <div class="flex items-center gap-3">
                        <div class="inline-flex h-10 w-10 items-center justify-center rounded-full bg-surface-container-highest text-outline text-sm font-semibold">
                    ...
                    <div class="text-left sm:text-center">
                        ...
                        <span class="inline-flex items-center rounded-full bg-secondary-fixed px-3 py-1 text-label-xs font-bold uppercase text-secondary">
                            Editor</span>
                        <?php else: ?>
                        <span class="inline-flex items-center rounded-full bg-surface-container-highest px-3 py-1 text-label-xs font-bold uppercase text-outline">
                            Member</span>
                        <?php endif; ?>
                    </div>
