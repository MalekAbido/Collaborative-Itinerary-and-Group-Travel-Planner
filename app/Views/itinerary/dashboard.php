                ...
                <header class="mb-10 flex justify-between items-end">
                    <div>
                        <h1 class="font-display text-display text-on-surface mb-2">
                            <?php echo htmlspecialchars($data['trip']['title']) ?></h1>
                        <p class="text-body-lg text-on-surface-variant flex items-center gap-2">
                            <span class="material-symbols-outlined text-[20px]">calendar_month</span>
                            <?php echo htmlspecialchars($data['trip']['startDate']) ?> to
                            <?php echo htmlspecialchars($data['trip']['endDate']) ?>
                        </p>
                    </div>
                    <div class="flex gap-3">
                        <?php
                        if (App\Services\Auth::hasRole(TripMemberRole::ORGANIZER->value, $userRole)): ?>
                        <a class="cursor-pointer inline-flex items-center gap-2 rounded-lg border-2 border-outline-variant text-on-surface font-semibold text-body-sm px-6 py-2.5 hover:bg-surface-container transition"    href="/itinerary/settings/<?php echo htmlspecialchars($data['trip']['id']) ?>"  >
                            <span class="material-symbols-outlined text-[18px]">settings</span> Settings
                        </a>
                        <?php endif; ?>
                        <a class="cursor-pointer inline-flex items-center gap-2 rounded-lg bg-primary text-on-primary font-semibold text-body-sm px-6 py-2.5 shadow-sm hover:bg-on-primary-fixed-variant transition"    href="/itinerary/<?php echo htmlspecialchars($data['trip']['id']) ?>/activity/create"
                             >
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
                    ...
                    <div class="sm:col-span-2 bg-surface-container-lowest border border-outline-variant rounded-xl shadow-sm p-5">
                        ...
                        <div class="text-body-md text-on-surface font-medium">
                            <?php echo htmlspecialchars($data['trip']['description'] ?: 'No description provided.') ?></div>
                    </div>
                    ...
                    <div class="sm:col-span-1 bg-surface-container-lowest border border-outline-variant rounded-xl shadow-sm p-5">
                        ...
                        <div class="font-display text-[28px] font-extrabold text-on-surface"><?php echo count($data['members']) ?></div>
                    </div>
                </div>
                                ...
                                foreach ($data['activities'] as $activity): ?>
                                    ...
                                    <div class="mb-8 ml-8 relative group">
                                        ...
                                        <div class="block bg-surface-container-lowest border border-outline-variant rounded-xl shadow-sm p-5 hover:shadow-md transition relative">
                                            ...
                                            <a class="cursor-pointer absolute inset-0 z-0 rounded-xl focus:outline-none"    href="/itinerary/<?php echo htmlspecialchars($data['trip']['id']) ?>/activity/<?php echo $activity->getId() ?>"  >
                                            ...
                                            <div class="flex flex-col sm:flex-row justify-between items-start gap-4">
                                                <div class="min-w-0 flex-1">
                                                    <h3 class="font-display text-h4 text-on-surface mb-1 group-hover:text-primary transition-colors">
                                                        <?php echo htmlspecialchars($activity->getName()) ?>
                                                    </h3>
                                                    ...
                                                    <div class="flex flex-wrap items-center text-body-sm text-on-surface-variant font-medium gap-x-4 gap-y-2 mt-2">
                                                        ...
                                                        <div class="flex items-center gap-1 whitespace-nowrap">
                                                            ...
                                                            <?php
                                                            if ($isSameDay): ?>
                                                                <?php echo date('M d, Y', $start) ?> • <?php echo date('h:i A', $start) ?> - <?php echo date('h:i A', $end) ?>
                                                            <?php else: ?>
                                                                <?php echo date('M d, h:i A', $start) ?> - <?php echo date('M d, h:i A', $end) ?>
                                                            <?php endif; ?>
                                                        </div>

                                                        <?php
                                                        if ($activity->getLocation() && $activity->getLocation()->getName()): ?>
                                                        <div class="flex items-center gap-1 min-w-0">
                                                            <span class="material-symbols-outlined text-[16px] shrink-0">location_on</span>
                                                            <span class="truncate"><?php echo htmlspecialchars($activity->getLocation()->getName()) ?></span>
                                                        </div>
                                                        <?php endif; ?>
                                                ...
                                                <div class="flex items-center gap-3 relative z-10 shrink-0">
                                                    <span class="inline-flex items-center rounded-full bg-primary-fixed px-3 py-1 text-label-xs font-bold uppercase text-primary">
                                                        <?php echo htmlspecialchars($activity->getCategory()) ?>
                                                    </span>

                                                    <?php
                                                    if ($userRole !== TripMemberRole::MEMBER->value): ?>
                                                        <form action="/itinerary/<?php echo htmlspecialchars($data['trip']['id']) ?>/activity/<?php echo $activity->getId() ?>/delete" method="POST" onsubmit="return confirm('Are you sure you want to delete this activity?');" class="m-0">
                                            ...
                                            if (! empty($activity->getDescription())): ?>
                                                <p class="text-body-md text-on-surface mt-4 pt-4 border-t border-outline-variant/50 relative z-10">
                                                    <?php echo nl2br(htmlspecialchars($activity->getDescription())) ?>
                                                </p>
                                            <?php endif; ?>
                                        </div>
                                    </div>
                ...
                <div class="space-y-8">
                    <div>
                        <div class="flex items-center justify-between pb-3 mb-4 border-b border-outline-variant">
                            ...
                            <a class="cursor-pointer text-body-sm font-semibold text-primary hover:underline"    href="/itinerary/members/<?php echo htmlspecialchars($data['trip']['id']) ?>"  >Manage</a>
                        </div>
                                    ...
                                    foreach ($data['members'] as $member): ?>
                                        <div class="flex items-center justify-between py-3 first:pt-0 last:pb-0">
                                            <div class="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <div class="font-display text-h4 text-on-surface"><?php echo htmlspecialchars($member['firstName'] . ' ' . $member['lastName']) ?></div>
