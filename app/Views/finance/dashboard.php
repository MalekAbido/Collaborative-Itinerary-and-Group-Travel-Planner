        ...
        theme: {
            extend: {
                colors: {
                    ...
                    "on-tertiary": "#ffffff",
                    "on-tertiary-fixed-variant": "#128200",
                    "success": "#16a34a",
...
<body>
            ...
            <div class="mx-auto flex h-full max-w-[1280px] items-center justify-between px-6 lg:px-8">
                <div class="flex items-center gap-8">
                    ...
                    <div class="hidden md:flex items-center gap-1">
                        ...
                        <a href="/itinerary/dashboard/<?= htmlspecialchars($tripStringId) ?>"
                        ...
                        <a href="/finance/dashboard/<?= htmlspecialchars($itineraryId) ?>"
                            class="px-3 py-2 rounded-md text-body-sm font-medium text-primary border-b-2 border-primary">Finances</a>
                    </div>
                </div>
            ...
            <div class="max-w-[1280px] mx-auto">
                ...
                <header class="mb-10 flex flex-col sm:flex-row sm:justify-between sm:items-end gap-4">
                    <div>
                        ...
                        <p class="text-body-lg text-on-surface-variant">Track group expenses and monitor budget limits.
                        </p>
                    </div>
                    <button type="button"
                        onclick="document.getElementById('financeSettingsModal').classList.remove('hidden')"
                ...
                <div
                    class="mb-8 flex items-center gap-3 rounded-xl border border-secondary/30 bg-secondary-fixed px-4 py-3 shadow-sm">
                ...
                <div
                    class="mb-8 flex items-center gap-3 rounded-xl border border-error/30 bg-error-container px-4 py-3 shadow-sm">
                    ...
                    <div class="flex-1">
                        ...
                        <h4 class="text-body-md font-semibold text-on-error-container m-0">
                            You have exceeded the total budget limit of
                            <?= htmlspecialchars($totalBudget . ' ' . $baseCurrency) ?>.
                        </h4>
                    </div>
                </div>
                <?php endif; ?>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
                    <div class="bg-surface-container-lowest border border-outline-variant rounded-xl shadow-sm p-5">
                        ...
                        <div class="font-display text-[28px] font-extrabold text-on-surface">
                            <?= number_format($actualSpending) ?> <span
                                class="text-h4 text-outline"><?= htmlspecialchars($baseCurrency) ?></span>
                        </div>
                    </div>
                    ...
                    <div class="bg-surface-container-lowest border border-outline-variant rounded-xl shadow-sm p-5">
                        <div class="flex items-start justify-between mb-3">
                            ...
                            <div
                                class="flex h-9 w-9 items-center justify-center rounded-full bg-surface-container-highest">
                                <span class="material-symbols-outlined text-[18px] text-outline">account_balance</span>
                            </div>
                        </div>
                        <div class="font-display text-[28px] font-extrabold text-on-surface">
                            <?= $totalBudget > 0 ? number_format($totalBudget) : 'No Limit' ?> <span
                                class="text-h4 text-outline"><?= htmlspecialchars($baseCurrency) ?></span>
                        </div>
                    </div>
                    ...
                    <div class="bg-surface-container-lowest border border-outline-variant rounded-xl shadow-sm p-5">
                        <div class="flex items-start justify-between mb-3">
                            ...
                            <div class="flex h-9 w-9 items-center justify-center rounded-full bg-success-container">
                                <span
                                    class="material-symbols-outlined text-[18px] text-on-success-container">savings</span>
                            </div>
                        </div>
                        <div
                            class="font-display text-[28px] font-extrabold <?= ($totalBudget - $actualSpending) < 0 ? 'text-error' : 'text-on-surface' ?>">
                            <?= $totalBudget > 0 ? number_format($totalBudget - $actualSpending) : '∞' ?> <span
                                class="text-h4 text-outline"><?= htmlspecialchars($baseCurrency) ?></span>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-surface-container-lowest border border-outline-variant rounded-xl shadow-sm p-6 mb-8">
                    ...
                    <div>
                        <div class="flex justify-between mb-2">
                            <span class="text-label-caps uppercase text-outline">0
                                <?= htmlspecialchars($baseCurrency) ?></span>
                            <span
                                class="text-body-sm font-semibold <?= $percentage >= 100 ? 'text-error' : 'text-primary' ?>">
                                <?= $percentage ?>% Used
                            </span>
                            <span class="text-label-caps uppercase text-outline"><?= number_format($totalBudget) ?>
                                <?= htmlspecialchars($baseCurrency) ?></span>
                        </div>
                ...
                <div class="mt-8 pt-8 border-t border-outline-variant">
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <div
                            class="bg-tertiary-fixed border border-outline-variant rounded-xl shadow-sm p-6 flex flex-col justify-center items-center text-center">
                            <span class="text-label-caps uppercase text-on-tertiary-fixed-variant mb-2">Current Pool
                                Balance</span>
                            <div class="font-display text-[40px] font-extrabold text-on-surface">
                                <?= number_format($kittyBalance ?? 0) ?> <span
                                    class="text-h3 text-on-surface-variant"><?= htmlspecialchars($baseCurrency) ?></span>
                            </div>
                            <p class="text-body-sm text-on-surface-variant mt-2">Available for central group expenses
                            </p>
                        </div>
                        ...
                        <div class="lg:col-span-2">
                            ...
                            <div
                                class="bg-surface-container-lowest border border-outline-variant rounded-xl shadow-sm p-5">
                                <h3 class="font-display text-h4 text-on-surface mb-1">Make a Contribution</h3>
                                <p class="text-body-xs text-on-surface-variant mb-4">Add your money to the central pool.
                                </p>

                                <form action="/fund/contribute/<?= htmlspecialchars($fundId) ?>" method="POST"
                                    class="flex flex-col gap-3">
                                    <input type="hidden" name="itineraryId"
                                        value="<?= htmlspecialchars($itineraryId) ?>">
                                    <input type="hidden" name="userId" value="1">
                                    <div class="flex items-center gap-2">
                                        <span
                                            class="text-body-md font-semibold text-outline"><?= htmlspecialchars($baseCurrency) ?></span>
                                    ...
                                    <button type="submit"
                                        class="inline-flex items-center justify-center gap-2 rounded-lg bg-tertiary text-on-tertiary font-semibold text-body-sm px-6 py-2.5 shadow-sm hover:bg-tertiary/90 transition">
                                        Add Funds
                                    </button>
                                </form>
                            </div>
                            ...
                            <div
                                class="mt-6 bg-surface-container-lowest border border-outline-variant rounded-xl shadow-sm overflow-hidden">
                                <div
                                    class="px-5 py-4 border-b border-outline-variant flex items-center justify-between">
                                    <h4 class="font-display text-body-lg font-semibold text-on-surface">Recent
                                        Contributions</h4>
                                    <span
                                        class="text-label-xs uppercase text-outline font-bold"><?= isset($contributions) ? count($contributions) : 0 ?>
                                        Records</span>
                                </div>
                                ...
                                <?php else: ?>
                                <ul
                                    class="divide-y divide-outline-variant/50 max-h-[300px] overflow-y-auto scroll-thin">
                                    <?php foreach ($contributions as $contribution): ?>
                                    <li
                                        class="px-5 py-3 flex items-center justify-between hover:bg-surface-container/50 transition">
                                        <div class="flex items-center gap-3">
                                            <div
                                                class="flex h-9 w-9 items-center justify-center rounded-full bg-tertiary-fixed text-tertiary text-xs font-bold border border-outline-variant/30">
                                        ...
                                        <div class="text-body-md font-bold text-on-surface">
                                            +<?= number_format($contribution['amount']) ?> <span
                                                class="text-body-xs font-normal text-outline"><?= htmlspecialchars($baseCurrency) ?></span>
                                        </div>
                                    </li>
                                    <?php endforeach; ?>
                                </ul>
                            ...
                            <?php else: ?>
                            <div
                                class="bg-surface-container-lowest border border-outline-variant rounded-xl shadow-sm p-5 flex flex-col justify-center items-center text-center text-on-surface-variant min-h-[160px]">
                                <span
                                    class="material-symbols-outlined text-[40px] mb-3 text-outline">account_balance_wallet</span>
                                ...
                                <form action="/finance/create-fund/<?= htmlspecialchars($itineraryId) ?>" method="POST">
                                    <button type="submit"
                                        class="inline-flex items-center gap-2 rounded-lg bg-tertiary text-on-tertiary font-semibold text-body-sm px-6 py-2.5 shadow-sm hover:bg-tertiary/90 transition">
                ...
                <div class="mt-8 pt-8 border-t border-outline-variant">
                    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                        ...
                        <a href="/finance/expense/add/<?= htmlspecialchars($itineraryId) ?>"
                    ...
                    <div
                        class="flex flex-col items-center justify-center py-16 border-2 border-dashed border-outline-variant rounded-xl bg-surface-container-lowest text-center">
                    ...
                    <div class="flex flex-col gap-4">
                        ...
                        <div
                            class="bg-surface-container-lowest border border-outline-variant rounded-xl shadow-sm p-5 flex flex-col hover:shadow-md transition">
                            <div class="flex justify-between items-start mb-2">
                                <div class="flex-1 pr-4">
                                    <h4 class="font-display text-h4 text-on-surface leading-tight mb-1">
                                        <?= htmlspecialchars($expense->getDescription()) ?></h4>
                                    <span
                                        class="inline-block bg-surface-container-highest px-2 py-0.5 rounded-md text-label-xs uppercase text-on-surface-variant tracking-wider">
                                        <?= htmlspecialchars($expense->getCategory()) ?>
                                    </span>
                                </div>
                                <div class="text-right shrink-0">
                                    <div class="font-display text-h3 font-extrabold text-on-surface">
                                        <?= number_format($expense->getAmount(), 2) ?> <span
                                            class="text-body-xs font-normal text-outline"><?= htmlspecialchars($expense->getCurrencyType()) ?></span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div
                                class="mt-3 bg-secondary-fixed text-on-secondary-fixed px-3 py-2.5 rounded-lg text-body-sm border border-secondary/20">
                                <div class="flex items-center gap-1.5 font-bold mb-0.5">
                                    ...
                                    Refund Applied: <?= number_format($expense->getRefundedAmount(), 2) ?>
                                    <?= htmlspecialchars($expense->getCurrencyType()) ?>
                                </div>
                                <div class="text-label-xs opacity-80 uppercase tracking-wide">
                                    Original Total:
                                    <?= number_format($expense->getAmount() + $expense->getRefundedAmount(), 2) ?>
                                    <?= htmlspecialchars($expense->getCurrencyType()) ?>
                                </div>
                            </div>
                            <?php endif; ?>
                            ...
                            <!-- Card Actions -->
                            <div
                                class="mt-auto pt-5 flex items-center justify-between gap-3 border-t border-outline-variant/50">
                                <a href="/finance/expense/details?id=<?= htmlspecialchars($expense->getId()) ?>"
                                    class="text-body-sm font-semibold text-primary hover:underline group flex items-center gap-1">
                                    View Details <span
                                        class="material-symbols-outlined text-[16px] group-hover:translate-x-0.5 transition-transform">arrow_forward</span>
                                </a>

                                <?php if ($expense->getAmount() > 0): ?>
                                <button
                                    onclick="openRefundModal(<?= htmlspecialchars($expense->getId()) ?>, <?= htmlspecialchars($expense->getAmount()) ?>)"
                                    class="inline-flex items-center gap-1.5 text-body-sm font-semibold text-outline hover:text-secondary transition">
                                    <span class="material-symbols-outlined text-[18px]">undo</span> Partial Refund
                                </button>
                                <?php else: ?>
                                <span
                                    class="inline-flex items-center gap-1 text-success text-body-sm font-bold bg-success-container px-2 py-1 rounded-md">
                                    <span class="material-symbols-outlined text-[16px]">check_circle</span> Fully
                                    Refunded
                                </span>
                                <?php endif; ?>
                            </div>
    ...
    <div id="financeSettingsModal"
        class="hidden fixed inset-0 z-[100] flex items-center justify-center bg-on-background/50 backdrop-blur-sm transition-opacity">
        <div
            class="w-full max-w-md rounded-2xl bg-surface-container-lowest shadow-lg border border-outline-variant overflow-hidden">
            <div class="flex items-center justify-between border-b border-outline-variant px-6 py-4 bg-surface">
                ...
                <button type="button" onclick="document.getElementById('financeSettingsModal').classList.add('hidden')"
                    class="flex h-8 w-8 items-center justify-center rounded-full text-on-surface-variant hover:bg-surface-container hover:text-on-surface transition">
                    <span class="material-symbols-outlined text-[20px]">close</span>
                </button>
            </div>
            <div class="p-6">
                <form action="/finance/update-settings/<?= htmlspecialchars($itineraryId) ?>" method="POST"
                    class="flex flex-col gap-5">
                    <div>
                        <label class="block text-label-caps uppercase text-on-surface-variant mb-2">Base
                            Currency</label>
                    ...
                    <div>
                        <label class="block text-label-caps uppercase text-on-surface-variant mb-2">Total Budget
                            Limit</label>
                        <input type="number" name="budgetLimit" min="0" step="0.01"
                            value="<?= htmlspecialchars($totalBudget) ?>" placeholder="0.00"
                            class="w-full rounded-md border border-outline-variant bg-surface px-3 py-2 text-body-md focus:border-primary focus:ring-primary focus:outline-none transition">
                        <p class="text-body-xs text-outline mt-2">Set to 0 for no limit.</p>
                    </div>
                    <div class="mt-2 flex justify-end gap-3">
                        <button type="button"
                            onclick="document.getElementById('financeSettingsModal').classList.add('hidden')"
                            class="px-5 py-2.5 rounded-lg border border-outline-variant text-on-surface font-semibold text-body-sm hover:bg-surface-container transition">
                            Cancel
                        </button>
                        <button type="submit"
                            class="inline-flex items-center justify-center gap-2 rounded-lg bg-primary text-on-primary font-semibold text-body-sm px-6 py-2.5 shadow-sm hover:bg-on-primary-fixed-variant transition">
        ...
        <div
            ...
            <div class="p-6">
                <form action="/finance/expense/refund" method="POST" class="flex flex-col gap-5">
                    ...
                    <div>
                        <label class="block text-label-caps uppercase text-on-surface-variant mb-2">Amount to
                            Refund</label>
                        <div class="flex items-center gap-2">
                            <span
                                class="text-body-md font-semibold text-outline"><?= htmlspecialchars($baseCurrency ?? '$') ?></span>
                            <input type="number" name="refundAmount" id="refundAmountInput" min="0.01" step="0.01"
                        ...
                        <p class="text-body-xs text-outline mt-2 font-medium">Maximum refund available: <span
                                id="maxRefundDisplay" class="text-on-surface"></span></p>
                    </div>
                    ...
                    <div class="mt-2 flex justify-end gap-3">
                        ...
                        <button type="submit"
                            class="inline-flex items-center justify-center gap-2 rounded-lg bg-secondary text-on-secondary font-semibold text-body-sm px-6 py-2.5 shadow-sm hover:bg-secondary-container transition">
                            Process Refund
                        </button>
                    </div>
                </form>
